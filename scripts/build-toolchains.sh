...
if [ "${EC2FASTINSTALL}" = true ] ; then
    ...
    module_build riscv-gnu-toolchain --prefix="${RISCV}" --enable-multilib --with-cmodel=medany
...
SRCDIR="$(pwd)/software" module_all libgloss --prefix="${RISCV}/riscv64-unknown-elf" --host=riscv64-unknown-elf
